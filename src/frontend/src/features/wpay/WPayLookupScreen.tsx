...
interface WPayLookupScreenProps {
  onBack: () => void;
}

export default function WPayLookupScreen({ onBack }: WPayLookupScreenProps) {
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      ...
      // Record transaction in the store
      addTransaction({
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-lg p-8 space-y-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-foreground">W Pay</h1>
          <p className="text-sm text-muted-foreground text-center">
            Enter your 6-digit code to view and pay your order
          </p>
        </div>

        {/* Code input — only shown when no order loaded */}
        {!order && (
          <div className="space-y-3">
            <Label htmlFor="wpay-code">Order Code</Label>
              ...
              >
                ...
                <div className="w-full rounded-lg bg-accent/20 border border-border p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground pt-1">
                    ...
                    <span className="font-medium text-foreground">W Pay</span>
                  </div>
                </div>
